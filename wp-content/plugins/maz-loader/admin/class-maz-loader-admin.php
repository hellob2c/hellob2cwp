	...
	function admin_page_init() {
		if ( $this->page == 'maz-loader' ) {
			...
			if ( $this->action == 'delete' ) {
				...
				{
					$notice = __( 'Cannot validate request.', 'maz-loader' );
					MZLDR_Admin_Notice::add_flash_notice( $notice, 'error' );
					MZLDR_Helper::customAdminRedirect( 'maz-loader-list' );
					return;
				}

				if ( ! wp_verify_nonce( sanitize_text_field($_GET['_wpnonce']), 'mzldr-item-delete' ) )
				{
					$notice = __( 'Validation started but failed.', 'maz-loader' );
					MZLDR_Admin_Notice::add_flash_notice( $notice, 'error' );
					MZLDR_Helper::customAdminRedirect( 'maz-loader-list' );
					return;
				}
			...
			if ( $this->action == 'update-publish-status' ) {
				...
				if ( ! wp_verify_nonce( sanitize_text_field($_GET['_wpnonce']), 'mzldr-item-update-status' ) )
				{
					return;
				}
	...
	function show_rate_reminder() {
		if( get_transient( 'mzldr_rate_reminder' ) ) {
			...
			if( $current_date >= $start_date ) {
				$image = sprintf( esc_html( '%1$s' ), '<img src="' . esc_url(MZLDR_ADMIN_MEDIA_URL) . 'img/logo.svg" width="100" alt="MAZ Loader Plugin Avatar" />' );
				$message = sprintf( esc_html__( '%1$s Thank you for using our plugin, MAZ Loader! %2$s You rock! Could you please do me a BIG favor and give it a 5-star rating on WordPress? This will help me spread the word and boost my motivation. %3$s - %4$s Feataholic %5$s %6$s', 'maz-loader' ), '<b>', '&ndash;', '</br>', '<span>', '</span>', '</b>' );

...
class MZLDR_Form_Controller {
	...
	public function maz_loader_submission_response() {
		...
		if ( isset( $_POST['maz_loader_meta_nonce'] ) && check_admin_referer( 'maz_loader_meta_form_nonce', 'maz_loader_meta_nonce' ) && current_user_can('manage_options') )
		{
			// sanitize the input
			$posted_data = wp_unslash( $_POST );

			$submission_type   = isset( $posted_data['submission_type'] ) ? sanitize_key( $posted_data['submission_type'] ) : '';
			$action            = isset( $posted_data['action'] ) ? sanitize_key( $posted_data['action'] ) : '';
			$name              = isset( $posted_data['mzldr']['loader_name'] ) ? sanitize_text_field( $posted_data['mzldr']['loader_name'] ) : '';
			$fields            = isset( $posted_data['mzldr']['data'] ) ? $posted_data['mzldr']['data'] : '';
			$fields            = MZLDR_Helper::sanitize_fields_data( $fields );
			$loader_settings   = isset( $posted_data['mzldr']['loader_settings'] ) ? $posted_data['mzldr']['loader_settings'] : '';
			$loader_settings   = MZLDR_Helper::sanitize_loader_settings( $loader_settings );
			$loader_appearance = isset( $posted_data['mzldr']['loader_appearance'] ) ? $posted_data['mzldr']['loader_appearance'] : '';
			...
			if ( $submission_type == 'new' ) {
				...
				$loader_id = isset( $posted_data['mzldr']['loader_id'] ) ? sanitize_text_field( $posted_data['mzldr']['loader_id'] ) : '';
	...
	public function maz_loader_preview_response() {
		if ( isset( $_POST['maz_loader_meta_nonce'] ) && check_admin_referer( 'maz_loader_meta_form_nonce', 'maz_loader_meta_nonce' ) && current_user_can('manage_options') )
		{
			// sanitize input
			$posted_data = wp_unslash( $_POST );
			$data  = isset( $posted_data['mzldr'] ) ? $posted_data['mzldr'] : array();

			if ( empty( $data ) ) {
				return;
			}

			$submission_type = isset( $posted_data['submission_type'] ) ? sanitize_key( $posted_data['submission_type'] ) : 'new_field';
			$field_type      = isset( $posted_data['field_type'] ) ? sanitize_key( $posted_data['field_type'] ) : '';
			...
			$loader_new_field_id = isset( $posted_data['loader_new_field_id'] ) ? sanitize_key( $posted_data['loader_new_field_id'] ) : '';
			...
			if ( $submission_type == 'new_field' ) {
				...
				$duplicate_field_id = isset( $posted_data['duplicate_field_id'] ) ? sanitize_key( $posted_data['duplicate_field_id'] ) : '';

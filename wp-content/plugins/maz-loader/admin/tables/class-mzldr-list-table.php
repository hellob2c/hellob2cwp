...
class MZLDR_List_Table extends WP_List_Table {
	...
	public function column_published( $item ) {
		...
		$nonce = wp_create_nonce( 'mzldr-item-update-status' );
		?>
		<a href="?_wpnonce=<?php echo esc_attr($nonce); ?>&page=maz-loader-list&action=update-publish-status&new_status=<?php echo esc_attr( $new_status ); ?>&loader_id=<?php echo esc_attr( $item->id ); ?>" class="mzldr-toggle-switch-link">
	...
	public function column_actions( $item ) {
		$nonce = wp_create_nonce( 'mzldr-item-delete' );
		...
		$actions = array(
			...
			'delete'    => sprintf( '<a href="?_wpnonce=%s&page=maz-loader&action=delete&loader_id=%s" title="' . __( 'Delete Loader', 'maz-loader' ) . '" class="mzldr-button x-small mzldr-confirm-popup" data-mzldr-confirm="yes" data-mzldr-confirm-message="' . __( 'Are you sure you want to delete this loader? You can\'t undo this.', 'maz-loader' ) . '">%s</a>', esc_attr($nonce), esc_attr( $item->id ), __( '<span class="dashicons dashicons-dismiss"></span>', 'maz-loader' ) ),
		);
	...
	public function prepare_items() {
		...
		$data['order_by'] = 'ORDER BY ' . $orderby . ' ' . $order;

		$data['limit'] = 'LIMIT ' . intval( $per_page );

		$paged          = isset( $_REQUEST['paged'] ) ? max( 0, intval( $_REQUEST['paged'] - 1 ) * $per_page ) : 0;
		$data['offset'] = 'OFFSET ' . $paged;

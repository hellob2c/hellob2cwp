...
class MZLDR {
	...
	private function load_dependencies() {
		...
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-maz-loader-migrator.php';
	...
	private function define_admin_hooks() {
		...
		// Update Process
		$this->loader->add_action( 'plugins_loaded', $this, 'update_process' );
		...
		if (MZLDR_Loader_Block::canRender()) {
			...
			$this->loader->add_filter( 'block_categories_all', $loader_block, 'register_block_category', 10, 2 );
		...
		// Update Checker
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/ajax/UpdateNotice.php';
		$updateChecker = new UpdateNotice();
		$this->loader->add_action( 'admin_init', $updateChecker, 'init' );

		$this->show_review_reminder($plugin_admin);
	}
	...
	private function show_review_reminder($plugin_admin)
	{
		...
		if (!in_array($page, $allowed_pages) && !wp_doing_ajax())
		{
			return;
		}
